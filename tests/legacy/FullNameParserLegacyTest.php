...
require_once __DIR__ . '/../../src/FullNameParserLegacy.php';

class FullNameParserLegacyTest extends TestCase
{
    public function testBasicParsing()
    {
        $parser = new FullNameParserLegacy();
    ...
    {
        $this->expectException(NameParsingException::class);
        $config = new ParserConfig(false, false, 10);
        $parser = new FullNameParserLegacy($config);
        $parser->parse('This name is definitely longer than ten characters');
    }

    public function testPreserveCaseOption()
    {
        $config = new ParserConfig(false, true);
        $parser = new FullNameParserLegacy($config);
    ...
    public function testNameOrderSwitch()
    {
        ...
        // Default: first-last
        $parserDefault = new FullNameParserLegacy();
        ...
        $parserLF = new FullNameParserLegacy($config);
